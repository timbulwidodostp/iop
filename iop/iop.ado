...
*! version 2.4 26feb2014 F. Chavez Juarez & Isidro Soloaga
program define iop , rclass 
version 9.2
syntax varlist [if] [in] [fweight iweight] ///
			[, Type(string) Detail Shapley(str) SGRoups(str) Oaxaca(str) Logit ///
...
if("`shapley'"!=""){
	...
	if((`case'==1 & inlist("`shapley'","ws","pdb")) | (`case'==2 & "`shapley'"=="pdb_max") | (`case'==3 & inlist("`shapley'","fg1a","fg1r","fg2r"))){
		preserve
		iop_shapley  [`weight'`exp'] if `touse',stat(`shapley') indepvars(`indepvars') depvars(`depvar') gr(`sgroups')
...
version 9.2
syntax [anything] [iweight fweight] [if] [in], stat(str) Indepvars(str) Depvars(str) [GRoup(str)]
marksample touse2
tempfile orgdb
quietly{
save `orgdb'
